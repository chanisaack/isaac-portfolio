...
### Media
Each project has a `projects/media/<slug>/` folder: numbered images (`<slug>-1.jpg`, `-2.jpg`, etc.) plus, where applicable, `<slug>-video.mp4` and a `<slug>-video-poster.jpg` frame grab used as the video's poster attribute. Videos were transcoded from the original phone footage (H.264 mp4, faststart, scaled to a max of 1280px width, reasonable CRF) — originals were 15–115MB each; compressed versions are 3–12MB. Each project's `-1.jpg` file is also used as its homepage cover thumbnail (`.entry-media img` on the homepage links directly to `projects/media/<slug>/<slug>-1.jpg`).
